...
dag = DAG('facebook_to_rds', catchup=False, default_args=default_args, schedule_interval="0 * * * *")
...
def trigger_facebook_glue_job(**kwargs):
    ...
    configs = s3_client.list_objects(
        Bucket='harward-lake',
        Delimiter='/',
        Prefix='configuration/facebook/'
    )

    for con in configs['Contents']:
        if con['Key'] != 'configuration/facebook/':
    ...
    response = client.start_job_run(
        JobName='MarketingSpendTableToRDS', Arguments={
            '--CONFIGS': json.dumps(_config_arr)})

    is_running = True
    while is_running:
        status = client.get_job_run(
            JobName='MarketingSpendTableToRDS', RunId=response['JobRunId'])
...
def trigger_facebook_crawler(**kwargs):
    client = boto3.client(service_name='glue', region_name='us-west-2',
                          endpoint_url='https://glue.us-west-2.amazonaws.com')
...
t2 = PythonOperator(
    task_id='trigger_facebook_glue_job',
    provide_context=True,
    python_callable=trigger_facebook_glue_job,
    dag=dag)

t1 = PythonOperator(
    task_id='trigger_facebook_crawler',
    python_callable=trigger_facebook_crawler,
    provide_context=True,
    dag=dag)
