...
def create_dag():
    ...
    if account_id:
        ...
        with open("facebook_daily_dag_template.py", 'r') as template_content:
            ...
            content = template_content.read().replace("START_DATE", "datetime({year}, {month}, {day})".format(year=ddd.year,
                                                                                                              month=ddd.month,
                                                                                                              day=ddd.day))
            content = content.replace('DAG_NAME', "act_{account_id}_daily_run_dag".format(account_id=account_id))
            content = content.replace('VARIABLES_NAME', "act_{account_id}_dag_variables".format(account_id=account_id))
        ...
        with open("facebook_backlog_dag_template.py", 'r') as backlog_template_content:
            ...
            backlog_content = backlog_template_content.read().replace("START_DATE",
                                                                      "datetime({year}, {month}, {day})".format(
                                                                          year=backlog_ddd.year, month=backlog_ddd.month,
                                                                          day=backlog_ddd.day))
            backlog_content = backlog_content.replace('DAG_NAME',
                                                      "act_{account_id}_backlog_run_dag".format(account_id=account_id))
            backlog_content = backlog_content.replace('VARIABLES_NAME',
                                                      "act_{account_id}_dag_variables".format(account_id=account_id))

        with open("{BASE_PATH}act_{account_id}_backlog_dag.py".format(BASE_PATH=BASE_PATH, account_id=account_id),
                  'w') as backlog_dag_file:
            backlog_dag_file.write(backlog_content)

        with open("{BASE_PATH}config/{filename}.json".format(BASE_PATH=BASE_PATH, account_id=account_id, filename=file_name),
                  'w') as variable_file:
...
# @app.route('/delete_dag', methods=['GET'])
# def delete_dag():
#     account_id = "2116882338339744"
#     file_name = ""
#     with open("facebook_daily_dag_template.py", 'r') as template_content:
...
#     os.system(restart_command)
#
#     return 'Request For Dag Deletion has been sent'
