...
file_key_regex = 'stripe_dispute_{0}/'.format(account_name)
...
s3_folder = s3_folder_path.format(datasource_type=datasource_type, transaction_type='dispute',
...
def get_insight_from_stripe(ds, **kwargs):
    ...
    if r.exists(dag_name) == 1:
        filename = 'dispute_data_{0}.json'.format(timestamp)
        f = open(file_path + file_key_regex + filename, 'w+')
        new_data = stripe.Dispute.list(ending_before=r.hget(dag_name, "last_obj_id"))
        ...
        filename = 'dispute_data_{0}.json'.format(timestamp)
        f = open(file_path + file_key_regex + filename, 'w+')
        backlog_data = stripe.Dispute.list()
...
def upload_dispute_data_to_s3_bucket(**kwargs):
    date_key = date.today()
...
t3 = PythonOperator(
    task_id='upload_dispute_data_to_s3_bucket',
    python_callable=upload_dispute_data_to_s3_bucket,
